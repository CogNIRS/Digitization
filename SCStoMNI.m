%% Script that convers Spatial Coordinate System (SCS) into Montreal Neurological Insititute (MNI) system

addpath('Toolboxes')

% Digitizer file names
F{1,1}= "Data\M1\capM_p01_MotCorr_coord.csv"; % optode coordiantes csv
F{1,2}= "Data\M1\capM_p01_MotCorr_orig.csv"; % reference points coordinates csv
F{1,3}= "Data\channel_config.txt"; % channel configuration


% Spatial Registration
spm_fnirs_spatialpreproc_ui_nofnirs(F)

disp('---SCS to MNI DONE---')